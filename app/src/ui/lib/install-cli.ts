...
export const InstalledCLIPath = '/usr/local/bin/commitly'

/** The path to the packaged CLI. */
const PackagedPath = Path.resolve(__dirname, 'static', 'commitly.sh')
  ...
  return new Promise<void>((resolve, reject) => {
    fsAdmin.unlink(InstalledCLIPath, error => {
      if (error !== null) {
        reject(
          new Error(
            `Failed to remove file at ${InstalledCLIPath}. Authorization of Commitly Helper is required.`
          )
        )
        return
      }

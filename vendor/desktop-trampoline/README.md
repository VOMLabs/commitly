...
A cross-platform no-dependency C executable trampoline which lets Commitly
...
Commitly, and we only need to provide the credentials to Git when
required.
...
On the other side, we have Commitly - an Electron app that is primarily
...
With this trampoline, all this info can be passed from Commitly to Git,
and then back to Commitly via a TCP socket when Git requires us the user
...
powerful mechanism for Commitly to interoperate with Git and handle any
...
`GIT_ASKPASS` can live within the Commitly codebase instead of having
